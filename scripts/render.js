...
// Link pro Instagram: se o lugar tem handle confirmado vai direto;
// se não, abre busca no Google ("instagram nome do lugar buenos aires")
// pra evitar levar a perfil errado adivinhando handles.
function linkInstagram(lugar) {
  if (lugar.insta) {
    const handle = lugar.insta.replace(/^@/, "");
    return `https://www.instagram.com/${handle}/`;
  }
  return `https://www.google.com/search?q=${encodeURIComponent(`instagram ${lugar.nome} buenos aires`)}`;
}
...
export function lugarDetalheHTML(id) {
  ...
  return `
    ...
    <div class="detail-actions">
      ...
      <a class="mini-btn mini-btn--insta" href="${linkInstagram(l)}" target="_blank" rel="noopener">
